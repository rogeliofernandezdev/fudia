...
export function DashboardView(){
  ...
  return <>
    ...
    <section className="dashboard-grid">
      ...
      <article className="panel alerts"><header><div><small>REQUIERE ATENCIÓN</small><h2>Alertas operativas</h2></div><b>{alertItems.length}</b></header>{alertItems.length?alertItems.map(a=><Link className="alert-row" key={a.title} href={a.href}><span className={a.tone}><Icon name="alert"/></span><b>{a.title}<small>{a.count} · {a.detail}</small></b><Icon name="chevron" size={16}/></Link>):<div className="catalog-state"><span><Icon name="check"/></span><b>Sin alertas pendientes</b><p>La operación no tiene incidencias activas en este momento.</p></div>}</article>
    </section>
