...
export function ProfilePage(){
  ...
  if(profile.isLoading)return <><ProfileHeader/><section className="panel management"><p>Cargando perfil…</p></section></>;
  if(profile.isError)return <><ProfileHeader/><section className="panel management"><p>{profile.error.message}</p><Button kind="secondary" onClick={()=>profile.refetch()}>Reintentar</Button></section></>;
  if(!profile.data)return <><ProfileHeader/><section className="panel management"><p>No pudimos cargar tu perfil.</p></section></>;
    ...
    <section className="panel management standardized-management">
      <form onSubmit={event=>{event.preventDefault();save.mutate()}}>
        <div className="form-grid">
          ...
          <label>Correo electrónico<Input readOnly value={profile.email}/></label>
