...
type Option={value:string;label:string};type Address={id?:string;label:string;address:string;reference:string;district:string;city:string;countryCode:string;default:boolean};
type Customer={id:string;code:string;customerType:"person"|"company";displayName:string;documentType:string;documentNumber:string;phone:string;email:string;preferredChannel:string;preferences:string;notes:string;marketingConsent:boolean;vipOverride:boolean;visitCount:number;totalSpent:string;lastPurchaseAt:string|null;active:boolean;segment:string;addresses?:Address[]};
type Response={items:Customer[];total:number;channelOptions:Option[];segmentOptions:Option[];customerTypeOptions:Option[];documentTypeOptions:Option[]};
type Draft=Pick<Customer,"customerType"|"displayName"|"documentType"|"documentNumber"|"phone"|"email"|"preferredChannel"|"preferences"|"notes"|"marketingConsent"|"vipOverride">&{id?:string;addresses:Address[]};
const empty:Draft={customerType:"person",displayName:"",documentType:"DNI",documentNumber:"",phone:"",email:"",preferredChannel:"none",preferences:"",notes:"",marketingConsent:false,vipOverride:false,addresses:[]};
const segmentLabels:Record<string,string>={new:"Nuevo",recurrent:"Recurrente",frequent:"Frecuente",vip:"VIP"};

export function CustomersManager(){const qc=useQueryClient();const{notify}=useFeedback();const settings=useSettings();const{can}=useSession();const canManage=can("customers.manage");const[q,setQ]=useState("");const[status,setStatus]=useState("");const[segment,setSegment]=useState("");const[page,setPage]=useState(1);const[size,setSize]=useState(10);const[draft,setDraft]=useState<Draft|null>(null);const[detailId,setDetailId]=useState<string|null>(null);const[target,setTarget]=useState<Customer|null>(null);
 const list=useQuery({queryKey:["customers",q,status,segment,page,size],queryFn:()=>apiFetch<Response>(`customers?q=${encodeURIComponent(q)}&status=${status}&segment=${segment}&page=${page}&pageSize=${size}`)});const detail=useQuery({queryKey:["customer",detailId],queryFn:()=>apiFetch<Customer>(`customers/${detailId}`),enabled:Boolean(detailId)});
 const save=useMutation({mutationFn:(v:Draft)=>apiFetch<Customer>(v.id?`customers/${v.id}`:"customers",{method:v.id?"PATCH":"POST",body:JSON.stringify(v)}),onSuccess:()=>{setDraft(null);void qc.invalidateQueries({queryKey:["customers"]});notify({tone:"success",title:"Cliente guardado",message:"Los datos del cliente quedaron actualizados."})},onError:e=>notify({tone:"danger",title:"No se pudo guardar",message:e.message})});
 const changeStatus=useMutation({mutationFn:(c:Customer)=>apiFetch<void>(`customers/${c.id}/status`,{method:"PATCH",body:JSON.stringify({active:!c.active})}),onSuccess:()=>{setTarget(null);void qc.invalidateQueries({queryKey:["customers"]});notify({tone:"success",title:"Estado actualizado",message:"El cliente conserva su historial y preferencias."})},onError:e=>notify({tone:"danger",title:"No se pudo actualizar",message:e.message})});
 async function edit(id:string){try{const c=await apiFetch<Customer>(`customers/${id}`);setDraft({id:c.id,customerType:c.customerType,displayName:c.displayName,documentType:c.documentType,documentNumber:c.documentNumber,phone:c.phone,email:c.email,preferredChannel:c.preferredChannel,preferences:c.preferences,notes:c.notes,marketingConsent:c.marketingConsent,vipOverride:c.vipOverride,addresses:c.addresses??[]})}catch(e){notify({tone:"danger",title:"No se pudo cargar",message:(e as Error).message})}}
 const items=list.data?.items??[];return <><PageHeader eyebrow="NEGOCIO" title="Clientes" description="Directorio maestro, preferencias e historial de relación con tus clientes." action={canManage?<Button icon="plus" onClick={()=>setDraft({...empty})}>Nuevo cliente</Button>:undefined}/><section className="panel management standardized-management customers-panel"><div className="toolbar"><label><Icon name="search" size={18}/><input value={q} onChange={e=>{setQ(e.target.value);setPage(1)}} placeholder="Buscar por nombre, documento, teléfono o correo..."/></label><select aria-label="Filtrar por segmento" value={segment} onChange={e=>{setSegment(e.target.value);setPage(1)}}><option value="">Todos los segmentos</option>{(list.data?.segmentOptions??[]).map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select><select aria-label="Filtrar por estado" value={status} onChange={e=>{setStatus(e.target.value);setPage(1)}}><option value="">Todos los estados</option><option value="active">Activos</option><option value="inactive">Inactivos</option></select></div>
 {list.isLoading?<Loading/>:list.isError?<State icon="alert" title="No pudimos cargar los clientes" text={list.error.message} action={()=>list.refetch()}/>:!items.length?<State icon="users" title="Aún no hay clientes" text={canManage?"Registra el primer cliente o ajusta los filtros.":"No hay clientes que coincidan con los filtros."} action={canManage?()=>setDraft({...empty}):undefined}/>:<><div className="table-wrap hover-scroll"><table><thead><tr><th>CLIENTE</th><th>CONTACTO</th><th>SEGMENTO</th><th>ACTIVIDAD</th><th>ESTADO</th><th>ACCIONES</th></tr></thead><tbody>{items.map((c,i)=><tr className={i%2?"alternate":""} key={c.id}><td><span className={`row-icon r${i%3}`}><Icon name="users" size={18}/></span><b>{c.displayName}</b>{c.documentNumber?<small className="doc-beside">{c.documentType} {c.documentNumber}</small>:null}</td><td>{c.phone||c.email||"Sin contacto"}<small>{c.phone&&c.email?c.email:""}</small></td><td><Status tone={c.segment==="vip"?"blue":c.segment==="frequent"?"green":"gray"}>{segmentLabels[c.segment]}</Status></td><td><b>{c.visitCount} visitas</b><small>S/ {Number(c.totalSpent).toFixed(2)} acumulado</small></td><td><Status tone={c.active?"green":"gray"}>{c.active?"Activo":"Inactivo"}</Status></td><td><div className="table-actions"><RowActionButton action="view" onClick={()=>setDetailId(c.id)}/>{canManage&&<><RowActionButton action="edit" onClick={()=>void edit(c.id)}/><RowActionButton action={c.active?"deactivate":"activate"} onClick={()=>setTarget(c)}/></>}</div></td></tr>)}</tbody></table></div><div className="management-cards">{items.map(c=><article key={c.id}><header><span className="row-icon r0"><Icon name="users"/></span><div><b>{c.displayName}</b>{c.documentNumber?<small className="doc-beside">{c.documentType} {c.documentNumber}</small>:null}</div><Status tone={c.active?"green":"gray"}>{c.active?"Activo":"Inactivo"}</Status></header><dl><div><dt>Contacto</dt><dd>{c.phone||c.email||"Sin contacto"}</dd></div><div><dt>Segmento</dt><dd>{segmentLabels[c.segment]}</dd></div><div><dt>Actividad</dt><dd>{c.visitCount} visitas · S/ {Number(c.totalSpent).toFixed(2)}</dd></div></dl><footer><RowActionButton action="view" onClick={()=>setDetailId(c.id)}/>{canManage&&<><RowActionButton action="edit" onClick={()=>void edit(c.id)}/><RowActionButton action={c.active?"deactivate":"activate"} onClick={()=>setTarget(c)}/></>}</footer></article>)}</div></>}
 <Pagination page={page} size={size} total={list.data?.total??0} onPage={setPage} onSize={v=>{setSize(v);setPage(1)}}/></section>{draft&&<CustomerDialog initial={draft} channels={list.data?.channelOptions??[]} busy={save.isPending} close={()=>setDraft(null)} save={v=>save.mutate(v)}/>} {detailId&&<CustomerDetail loading={detail.isLoading} customer={detail.data} error={detail.error?.message} channels={list.data?.channelOptions??[]} currencySymbol={settings.currencySymbol} close={()=>setDetailId(null)}/>}<ConfirmDialog open={Boolean(target)} title={target?.active?"Desactivar cliente":"Activar cliente"} description={target?.active?`“${target?.displayName??""}” dejará de estar disponible; su historial se conservará.`:`“${target?.displayName??""}” volverá a estar disponible.`} tone={target?.active?"danger":"success"} confirmLabel={target?.active?"Desactivar":"Activar"} pending={changeStatus.isPending} onCancel={()=>setTarget(null)} onConfirm={()=>target&&changeStatus.mutate(target)}/></>}

function CustomerDialog({initial,channels,busy,close,save}:{initial:Draft;channels:Option[];busy:boolean;close:()=>void;save:(v:Draft)=>void}){
 ...
 return <div className="modal-backdrop modal-overlay-in"><section className="crud-modal customer-modal modal-panel-in" role="dialog" aria-modal="true"><div className="modal-accent"/>
  ...
  <form onSubmit={e=>{e.preventDefault();save(v)}}><div className="customer-form-body">
   <div className="form-grid customer-main-grid">
    <label>Tipo<Select value={v.customerType} onChange={e=>setV({...v,customerType:e.target.value as Draft["customerType"]})}><option value="person">Persona</option><option value="company">Empresa</option></Select></label>
