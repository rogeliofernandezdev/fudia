...
function PurchaseDetail({order,mode,canManage,canReceive,busy,currency,country,timezone,close,edit,changeStatus,receive,cancel}:{order:PurchaseOrder;mode:"view"|"review";canManage:boolean;canReceive:boolean;busy:boolean;currency:string;country?:string|null;timezone?:string|null;close:()=>void;edit:()=>void;changeStatus:(next:"draft"|"pending_approval"|"approved"|"cancelled")=>void;receive:()=>void;cancel:()=>void}){
  ...
  return <div className="modal-backdrop modal-overlay-in"><section className="crud-modal purchase-detail-modal modal-panel-in" role="dialog" aria-modal="true" aria-labelledby="purchase-detail-title" aria-busy={busy}><div className="modal-accent"/><header><span className="modal-title-icon"><Icon name="receipt" size={18}/></span><div><small>{mode==="view"?"DETALLE DE ORDEN":"REVISAR ORDEN"}</small><h2 id="purchase-detail-title">{order.number}</h2></div><button type="button" aria-label="Cerrar" onClick={close} disabled={busy}><Icon name="close"/></button></header><div className="purchase-detail-body">
    ...
    {mode==="review"&&(canManage||canReceive)&&order.status!=="received"&&order.status!=="cancelled"&&<section className="purchase-detail-actions"><div><small>SIGUIENTE PASO</small><b>{nextStepLabel(order.status)}</b></div><div>{canManage&&order.status==="draft"&&<><Button kind="secondary" icon="edit" onClick={edit} disabled={busy}>Editar</Button><Button icon="chevron" onClick={()=>changeStatus("pending_approval")} disabled={busy}>Enviar a aprobación</Button></>}{canManage&&order.status==="pending_approval"&&<><Button kind="ghost" icon="chevronLeft" onClick={()=>changeStatus("draft")} disabled={busy}>Volver a borrador</Button><Button icon="check" onClick={()=>changeStatus("approved")} disabled={busy}>Aprobar</Button></>}{canReceive&&receivable&&<Button kind="success" icon="stock" onClick={receive} disabled={busy}>{order.status==="partially_received"?"Continuar recepción":"Registrar recepción"}</Button>}{canManage&&cancellable&&<Button kind="danger" icon="close" onClick={cancel} disabled={busy}>Cancelar orden</Button>}</div></section>}
