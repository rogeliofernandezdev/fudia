...
import {getPurchaseOrder,listPurchaseInventory,listPurchaseOrders,listSuppliers,receivePurchaseOrder,savePurchaseOrder,saveSupplier,setPurchaseOrderStatus,setSupplierActive} from "../infrastructure/purchases-api";
...
export function PurchasesPage(){
  ...
  const transition=useMutation({mutationFn:({id,next}:{id:string;next:Exclude<PurchaseStatus,"received">})=>setPurchaseOrderStatus(id,next),onSuccess:(_,variables)=>{void qc.invalidateQueries({queryKey:["purchase-orders"]});void qc.invalidateQueries({queryKey:["purchase-order"]});void qc.invalidateQueries({queryKey:["dashboard"]});if(variables.next==="cancelled"){setCancelTarget(null)}notify({tone:"success",title:"Estado actualizado",message:"La orden de compra quedó actualizada."})},onError:error=>notify({tone:"danger",title:"No se pudo actualizar la orden",message:error.message})});
  const receive=useMutation({mutationFn:receivePurchaseOrder,onSuccess:result=>{void qc.invalidateQueries({queryKey:["purchase-orders"]});void qc.invalidateQueries({queryKey:["purchase-order"]});void qc.invalidateQueries({queryKey:["inventory"]});void qc.invalidateQueries({queryKey:["inventory-products"]});void qc.invalidateQueries({queryKey:["inventory-movements"]});void qc.invalidateQueries({queryKey:["dashboard"]});notify({tone:"success",title:"Compra recibida",message:`${result.number} ingresó al Inventario y al Kárdex del local.`})},onError:error=>notify({tone:"danger",title:"No se pudo recibir la compra",message:error.message})});
  ...
  return <>
    ...
    {orderDraft&&(orderDraftLoading||activeSuppliers.isLoading||inventory.isLoading?<RemoteModalSkeleton className="purchase-order-modal" label="Cargando datos de compra" rows={7} close={()=>{setOrderDraft(null);setOrderDraftLoading(false)}}/>:<PurchaseOrderDialog initial={orderDraft} suppliers={activeSuppliers.data?.items??[]} inventory={inventory.data?.items??[]} currencySymbol={settings.currencySymbol} busy={saveOrder.isPending} close={()=>setOrderDraft(null)} save={draft=>saveOrder.mutate(draft)}/>)}
...
function PurchaseOrderDialog({initial,suppliers,inventory,currencySymbol,busy,close,save}:{initial:PurchaseOrderDraft;suppliers:Supplier[];inventory:PurchaseInventoryOption[];currencySymbol:string;busy:boolean;close:()=>void;save:(draft:PurchaseOrderDraft)=>void}){
  ...
  function chooseItem(index:number,id:string){
    const option=inventory.find(item=>item.id===id);
  ...
  return <div className="modal-backdrop modal-overlay-in"><section className="crud-modal purchase-order-modal modal-panel-in" role="dialog" aria-modal="true" aria-labelledby="purchase-order-title" aria-busy={busy}><div className="modal-accent"/>
    ...
    <form onSubmit={handleSubmit(save)} noValidate><div className="purchase-form-body">
      <section className="purchase-form-section"><div className="purchase-section-title"><span><Icon name="truck" size={17}/></span><div><b>Proveedor y entrega</b><small>Define quién abastece la orden y cuándo esperas recibirla.</small></div></div><div className="form-grid">
        <label className="span-2">Proveedor<Select autoFocus {...register("supplierId")} aria-invalid={Boolean(errors.supplierId)}><option value="">{suppliers.length?"Selecciona un proveedor":"No hay proveedores activos"}</option>{suppliers.map(supplier=><option value={supplier.id} key={supplier.id}>{supplier.name}{supplier.taxId?` · ${supplier.taxId}`:""}</option>)}</Select>{errors.supplierId?.message&&<small className="wizard-field-error">{errors.supplierId.message}</small>}</label>
      ...
      <section className="purchase-form-section"><div className="purchase-section-title purchase-lines-title"><span><Icon name="stock" size={17}/></span><div><b>Artículos de la orden</b><small>El costo se registra por presentación de compra.</small></div><Button kind="secondary" icon="plus" onClick={()=>append({inventoryItemId:"",presentationId:"",quantity:"1",unitCost:""})}>Agregar línea</Button></div>
        <div className="purchase-lines">{fields.map((field,index)=>{const value=lines[index];const option=inventory.find(item=>item.id===value?.inventoryItemId);return <article className="purchase-line" key={field.id}>
          <div className="purchase-line-number">{index+1}</div>
          <label>Artículo<Select value={value?.inventoryItemId??""} onChange={event=>chooseItem(index,event.target.value)} aria-invalid={Boolean(errors.items?.[index]?.inventoryItemId)}><option value="">Selecciona...</option>{inventory.map(item=><option value={item.id} key={item.id}>{item.name}{item.kind==="ingredient"?" · Insumo":""}</option>)}</Select>{errors.items?.[index]?.inventoryItemId?.message&&<small className="wizard-field-error">{errors.items[index]?.inventoryItemId?.message}</small>}</label>
          <label>Presentación<Select {...register(`items.${index}.presentationId`)} disabled={!option} aria-invalid={Boolean(errors.items?.[index]?.presentationId)}><option value="">Selecciona...</option>{(option?.presentations??[]).map(presentation=><option value={presentation.id} key={presentation.id}>{presentationName(presentation.presentationType,presentation.unitsPerPresentation,option?.unit)}</option>)}</Select>{errors.items?.[index]?.presentationId?.message&&<small className="wizard-field-error">{errors.items[index]?.presentationId?.message}</small>}</label>
...
function PurchaseTableSkeleton(){return <div className="purchases-table-wrap"><table aria-label="Cargando compras"><thead><tr><th>ORDEN</th><th>PROVEEDOR</th><th>FECHA</th><th>TOTAL</th><th>ESTADO</th><th>ACCIONES</th></tr></thead><tbody>{Array.from({length:6},(_,row)=><tr className="purchase-skeleton" key={row}>{Array.from({length:6},(_,cell)=><td key={cell}><i/></td>)}</tr>)}</tbody></table></div>}
function SupplierTableSkeleton(){return <div className="purchases-table-wrap"><table aria-label="Cargando proveedores"><thead><tr><th>PROVEEDOR</th><th>RUC</th><th>CONTACTO</th><th>ESTADO</th><th>ACCIONES</th></tr></thead><tbody>{Array.from({length:6},(_,row)=><tr className="purchase-skeleton" key={row}>{Array.from({length:5},(_,cell)=><td key={cell}><i/></td>)}</tr>)}</tbody></table></div>}
