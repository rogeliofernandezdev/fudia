...
export function ConciergeSettingsPage(){
 ...
 const[draft,setDraft]=useState<ConciergeSettingsDraft|null>(null);
 const value:ConciergeSettingsDraft=draft??{active:query.data?.active??false,whatsappPhone:query.data?.whatsappPhone??""};
 ...
 const phoneValid=value.whatsappPhone===""||/^\+[1-9][0-9]{7,14}$/.test(value.whatsappPhone);
 const canSave=canManage&&phoneValid&&(!value.active||value.whatsappPhone!=="")&&!mutation.isPending;
  ...
  <div className="concierge-settings">
   {query.isLoading?<section className="panel concierge-loading">Cargando configuración de Fudia Concierge…</section>:query.isError?<section className="panel concierge-error"><Icon name="alert" size={26}/><b>No pudimos cargar Fudia Concierge</b><p>{query.error.message}</p><Button kind="secondary" icon="refresh" onClick={()=>query.refetch()}>Reintentar</Button></section>:query.data&&<>
    <section className="panel concierge-overview">
     ...
     <span className={`concierge-state ${value.active?"active":""}`}><i/>{value.active?"Activo":"Inactivo"}</span>
    </section>
    <div className="concierge-config">
     <form className="panel concierge-form" onSubmit={event=>{event.preventDefault();if(canSave)mutation.mutate({...value,whatsappPhone:value.whatsappPhone.trim()})}}>
      ...
      <label className="concierge-field">Número de WhatsApp
       <Input disabled={!canManage} value={value.whatsappPhone} onChange={e=>setDraft({...value,whatsappPhone:e.target.value.replace(/\s/g,"")})} placeholder="+51987654321" inputMode="tel"/>
      ...
      <label className="concierge-toggle">
       <input disabled={!canManage} type="checkbox" checked={value.active} onChange={e=>setDraft({...value,active:e.target.checked})}/>
      ...
      {value.active&&!value.whatsappPhone&&<div className="concierge-warning">Configura el número de WhatsApp antes de activar Concierge.</div>}
