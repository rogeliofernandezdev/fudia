...
var defaultOrganizationRoles = []organizationRoleSeed{
	...
	{Name:"Gerente de local",SystemKey:"location_manager",Description:"Gestiona la operación completa de los locales asignados",MenuAccess:[]string{"dashboard","pos","pedidos","cocina","mesas","caja","reservas","productos","disponibilidad","combos","recetas","inventario","kardex","compras","clientes","locales","costos","bi","fiscal","usuarios","facturacion","integraciones"},Permissions:[]string{"dashboard.read","menu.read","menu.manage","customers.read","customers.manage","orders.read","orders.manage","cash.read","cash.manage","inventory.read","purchases.read","reports.read"}},
	{Name:"Supervisor de turno",SystemKey:"shift_supervisor",Description:"Supervisa atención, comandas, mesas, caja y turnos",MenuAccess:[]string{"dashboard","pos","pedidos","cocina","mesas","caja","reservas","productos","disponibilidad","combos","clientes"},Permissions:[]string{"dashboard.read","menu.read","customers.read","customers.manage","orders.read","orders.manage","cash.read","cash.manage","tables.read","tables.manage"}},
	{Name:"Cajero",SystemKey:"cashier",Description:"Opera caja, cobros, comprobantes y arqueos",MenuAccess:[]string{"pos","pedidos","mesas","caja","clientes","facturacion"},Permissions:[]string{"menu.read","customers.read","customers.manage","orders.read","orders.manage","cash.read","cash.manage","receipts.manage"}},
	{Name:"Mesero",SystemKey:"waiter",Description:"Gestiona mesas, clientes, pedidos y comandas",MenuAccess:[]string{"pos","pedidos","mesas"},Permissions:[]string{"menu.read","customers.read","customers.manage","orders.read","orders.manage","tables.read"}},
	...
	{Name:"Compras",SystemKey:"buyer",Description:"Gestiona proveedores y órdenes de compra",MenuAccess:[]string{"inventario","kardex","compras","logistica"},Permissions:[]string{"inventory.read","purchases.read","purchases.manage","purchases.approve"}},
	{Name:"Repartidor",SystemKey:"courier",Description:"Consulta pedidos asignados y actualiza entregas",MenuAccess:[]string{"delivery"},Permissions:[]string{"delivery.read","delivery.manage"}},
	{Name:"Contabilidad",SystemKey:"accounting",Description:"Consulta cierres, comprobantes, impuestos y reportes",MenuAccess:[]string{"dashboard","caja","costos","bi","fiscal","facturacion"},Permissions:[]string{"cash.read","receipts.read","reports.read","fiscal.read"}},
	{Name:"Auditor",SystemKey:"auditor",Description:"Acceso de solo lectura a operación e historial",MenuAccess:[]string{"dashboard","productos","clientes","inventario","kardex","compras","costos","bi"},Permissions:[]string{"dashboard.read","menu.read","customers.read","orders.read","cash.read","inventory.read","purchases.read","reports.read","audit.read"}},
}
