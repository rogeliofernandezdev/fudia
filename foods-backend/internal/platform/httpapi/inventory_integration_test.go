...
func seedInventoryScope(t *testing.T, pool *pgxpool.Pool) scope {
	...
	t.Cleanup(func() {
		...
		_, _ = pool.Exec(context.Background(), `DELETE FROM purchase_receipts WHERE organization_id=$1`, organizationID)
		_, _ = pool.Exec(context.Background(), `DELETE FROM purchase_orders WHERE organization_id=$1`, organizationID)
		_, _ = pool.Exec(context.Background(), `DELETE FROM suppliers WHERE organization_id=$1`, organizationID)
		...
		_, _ = pool.Exec(context.Background(), `DELETE FROM user_roles WHERE user_id IN (SELECT id FROM users WHERE organization_id=$1)`, organizationID)
		_, _ = pool.Exec(context.Background(), `DELETE FROM roles WHERE organization_id=$1`, organizationID)
