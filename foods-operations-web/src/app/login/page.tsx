...
export default function LoginPage() {
  ...
  const [email, setEmail] = useState("admin@saborcriollo.pe");
  const [password, setPassword] = useState("restaurante2026");
      ...
      <div className="login-card">
        ...
        <div className="login-card-body">
          ...
          <form className="login-form" onSubmit={async event => {
            ...
            <Label htmlFor="login-email">Correo electrónico<div className="auth-field"><Icon name="user"/><Input className="auth-input" id="login-email" autoComplete="username" placeholder="nombre@restaurante.com" value={email} onChange={event => setEmail(event.target.value)} type="email"/></div></Label>
            <Label htmlFor="login-password">Contraseña<div className="auth-field"><Icon name="lock"/><Input className="auth-input auth-password" id="login-password" autoComplete="current-password" placeholder="Ingresa tu contraseña" value={password} onChange={event => setPassword(event.target.value)} type={showPassword ? "text" : "password"}/><Button className="auth-eye" layout="icon" aria-label={showPassword ? "Ocultar contraseña" : "Mostrar contraseña"} aria-pressed={showPassword} onClick={() => setShowPassword(value => !value)}><Icon name="eye"/></Button></div></Label>
