...
no decoran tarjetas completas. Cada carril conserva borde y cuerpo neutros. La cabecera completa usa un fondo semántico claramente distinguible: azul `primary-600`, violeta `digital-500` o verde `brand-600`. Azul y violeta usan título/descripción en blanco; el verde `brand-600` usa `ink-950` para el título y `ink-600` para la descripción. Icono y contador se apoyan en superficie blanca y usan `brand-700` como acento. No se usan franjas superiores ni barras laterales de color.
